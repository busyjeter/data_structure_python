...
class LList1(LList):
	...
	def prepend(self,elem):# 表头添加
		if self._head==None: #如果先检查self._head是否None，那么head就应该检查后再创建
			self._head=LNode(elem,self._head)
	...
	def pop_last(self):
		#删除表尾结点
		if self._head ==None:#如果空表
			raise LinkedListUnderflow("in pop_last")
		p=self._head
		e=self._rear.elem
		if p.next==None:#如果仅一个元素,判断后必须跳出函数，不然会执行下面while报错
